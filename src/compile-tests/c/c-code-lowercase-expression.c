...
void assert_c_code_lowercase_expression_test(void) {
	...
	int a = (debug_assert(foo() == 2.5f), debug_assert(x > 5), 11 + x);
	...
	int c = (debug_assert(a + f < 7.0, "(message: %s %s @ %d)", "kukurutz", "shoobidoosah", 777), 2);

	debug_assert(a + b / c + f * 11 <= 7, s);
